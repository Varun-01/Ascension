...
public class AddMoney : MonoBehaviour {
	...
	void Awake() {
		...
		msgQueue.AddCallback(Constants.SMSG_ADDM, responseAddMoney);
	...
	public void Submit() {
		...
		cManager.send(requestAddMoney(moneyToAdd));  
	...
	public RequestAddMoney requestAddMoney(int moneyToAdd) {
		RequestAddMoney request = new RequestAddMoney();
		if(request != null) {Debug.Log ("request i AddMoney.cs is NOT null*******");}
		request.send(moneyToAdd);
		Debug.Log ("called requestAddMoney function and send");
		return request;
	}
	
	public void responseAddMoney(ExtendedEventArgs eventArgs) {
		
		ResponseAddMoneyEventArgs args = eventArgs as ResponseAddMoneyEventArgs;
		if (args.status == 0) {
			Constants.USER_ID = args.user_id;
			Debug.Log ("Successful addMoney response : " + args.money);
			EditorUtility.DisplayDialog ("AddMoney Successful", "You have successfully ADDMONEY in.\nClick Ok to continue execution and see responses on console", "Ok");
